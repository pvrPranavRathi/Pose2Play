...
def checkJumpCrouch(img, results, MID_Y=250, draw=False):
    ...
    actual_mid_x = abs(right_x + left_x) // 2
    
    # Detect if the user is leaning towards left or right while crouching
    if actual_mid_y > lower_bound and actual_mid_y < upper_bound:
        if left_x < actual_mid_x:
            posture = 'Crouch Left'
        else:
            posture = 'Crouch Right'
    elif actual_mid_y < lower_bound:
        posture = 'Jumping'
    else:
        posture = 'Standing'
    ...
    while True:
        ...
        if landmarks:
            ...
            if MID_Y:
                ...
                elif posture == 'Crouch Left' and y_pos_index == 1:
                    pyautogui.press('down')
                    y_pos_index -= 1 

                elif posture == 'Crouch Right' and y_pos_index == 1:
                    pyautogui.press('down')
                    y_pos_index -= 1 
